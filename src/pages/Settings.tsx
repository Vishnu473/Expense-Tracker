      ...
      <div className="max-w-4xl w-full mx-auto p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input id="name" aria-invalid={!!errors.name} aria-describedby={errors.name ? "name-error" : undefined}
              className={`border-gray-300 border dark:bg-gray-700 text-black dark:text-white p-2 rounded w-full focus:outline-none focus:ring-1 ${errors.name ? 'border border-red-500 focus:ring-red-500' : 'dark:focus:border-blue-600 focus:ring-blue-500'
                }`} {...register('name')} placeholder="Name" autoComplete="name" />
          ...
          <div>
            ...
            <input id="email" aria-invalid={!!errors.email} aria-describedby={errors.email ? "email-error" : undefined}
              className={`border-gray-300 border dark:bg-gray-700 text-black dark:text-white p-2 rounded w-full focus:outline-none focus:ring-1 ${errors.email ? 'border border-red-500 focus:ring-red-500' : 'dark:focus:border-blue-600 focus:ring-blue-500'
                }`} {...register('email')} placeholder="Email" autoComplete="email" />
          ...
          <div>
            ...
            <input id="phone" aria-invalid={!!errors.phone} aria-describedby={errors.phone ? "phone-error" : undefined}
              className={`border-gray-300 border dark:bg-gray-700 text-black dark:text-white p-2 rounded w-full focus:outline-none focus:ring-1 ${errors.phone ? 'border border-red-500 focus:ring-red-500' : ' dark:focus:border-blue-600 focus:ring-blue-500'
                }`} {...register('phone')} placeholder="Phone" autoComplete="tel" />
            {errors.phone && <p id="phone-error" className="text-red-500 text-sm">{errors.phone.message}</p>}
          </div>

          <button
            type="submit" aria-busy={isPending}
            className={clsx("bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700", isPending ? "disabled:bg-gray-500 disabled:cursor-not-allowed disabled:opacity-50" : "bg-blue-600 dark:bg-cyan-600")}
        ...
        <div className="mt-8">
          ...
          {/* <div className="flex justify-between items-center p-2 rounded">
            ...
            <input id="notifications" type="checkbox" aria-label="Enable notifications"/>
          </div> */}
